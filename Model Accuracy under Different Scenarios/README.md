...
6. Run 'test_example.sh'. Please uncomment the line comment and run 'test_example.sh' to get the corresponding results for the model. 

   ```$ sh test_example.sh``` 

   For the above example, we uncomment 'python all.py  -dataset mnist  -model lenet1' to get the results for LeNet1 model. The result will be stored in 'result.txt'.
